...
function formatGroupBuy(raw) {
  ...
  const canJoin = Boolean(raw.can_join ?? ((raw.status === 'pending' || raw.status === 'success') && !isExpired && stock > 0));
  return {
    ...raw,
    id: groupBuyId,
    ...
    can_join: canJoin,
    status_text: isSuccess ? '已成团' : (isExpired ? '已结束' : '待成团'),
    join_button_text: isSuccess ? '已成团，可继续购买' : '参团下单'
  };
}
