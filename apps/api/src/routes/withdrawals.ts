...
export function ensureTaxExportWithinLimit<T>(records: T[], limit = TAX_EXPORT_LIMIT) {
  ...
  app.get(
    ...
    async (request, reply) => {
      try {
        ...
        const context = resolveAdminAccessContext(request)!;
        const where = await buildTaxRecordWhere(query, context);
        const exportRecords = await prisma.taxRecord.findMany({ where, orderBy: [{ created_at: "desc" }, { id: "desc" }], take: TAX_EXPORT_LIMIT + 1 });
