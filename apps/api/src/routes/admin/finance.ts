...
import { getFinanceAfterSales, getFinanceOrders, getFinanceOverview, getFinanceRewards, toCsv } from '../../modules/finance/finance-report-service.js';

type Q = { from?: string; to?: string; community_id?: string; pickup_store_id?: string; status?: string; page?: string; page_size?: string; type?: string };
...
export function registerAdminFinanceRoutes(app: FastifyInstance) {
  ...
  app.get('/api/admin/finance/reconciliation/export.csv', async (request, reply) => {
    ...
    const rows = query.type === 'rewards' ? await getFinanceRewards(q(query)) : query.type === 'after_sales' ? await getFinanceAfterSales(q(query)) : query.type === 'overview' ? [await getFinanceOverview(q(query))] : (await getFinanceOrders(q(query))).items;
