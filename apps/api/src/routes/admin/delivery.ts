...
type DeliveryQuery = { status?: DeliveryStatus; provider?: DeliveryProvider; pickup_store_id?: string; keyword?: string; page?: string; page_size?: string };
...
export function registerAdminDeliveryRoutes(app: FastifyInstance) {
  app.get('/api/admin/delivery/orders', { preHandler: requireAdminPermission(['order.view', 'pickup.verify']) }, async (request, reply) => {
    try { return ok(await listDeliveryReservations(request.query as DeliveryQuery)); } catch (error) { reply.code(400); return fail(error instanceof Error ? error.message : '配送订单查询失败'); }
  });

  app.get('/api/admin/delivery/orders/:id', { preHandler: requireAdminPermission(['order.view', 'pickup.verify']) }, async (request, reply) => {
    try { return ok(await getDeliveryReservation((request.params as { id: string }).id)); } catch (error: any) { reply.code(error?.statusCode ?? 400); return fail(error instanceof Error ? error.message : '配送详情查询失败'); }
  });

  app.post('/api/admin/delivery/orders/:id/reserve', { preHandler: requireAdminPermission('order.manage') }, async (request, reply) => {
    try { return ok(await reserveDelivery((request.params as { id: string }).id, request.body as ReserveBody, actor(request))); } catch (error) { reply.code(400); return fail(error instanceof Error ? error.message : '配送预留失败'); }
  });

  app.post('/api/admin/delivery/orders/:id/status', { preHandler: requireAdminPermission('order.manage') }, async (request, reply) => {
    try { return ok(await updateDeliveryStatus((request.params as { id: string }).id, request.body as StatusBody, actor(request))); } catch (error) { reply.code(400); return fail(error instanceof Error ? error.message : '配送状态更新失败'); }
  });
