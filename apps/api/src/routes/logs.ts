...
function focusFrom(events: Array<{ event_type: string }>, alerts: Array<{ alert_type: string }>, commissions: Array<{ status: string }>) {
...
export function registerLogRoutes(app: FastifyInstance) {
  ...
  app.get('/api/admin/logs/orders/:order_id/ai-context', async (request, reply) => {
    ...
    const [timeline, businessEvents, alerts] = await Promise.all([
    ...
    return ok({
      ...
      alerts,
      suggested_focus: focusFrom(businessEvents, alerts, order.commissions)
    });
  });
}
