...
function whereOf(query: Query, context: AdminAccessContext): Prisma.OrderWhereInput | null { // scope 检查
...
export async function listDeliveryReservations(query: Query, context: AdminAccessContext) {
...
export async function getDeliveryReservation(id: string, context: AdminAccessContext) { const order = await prisma.order.findUnique({ where: { id }, include: { pickup_store: true, community: true } }); if (!order) throw Object.assign(new Error('订单不存在'), { statusCode: 404 }); if (!canAccessOrderDataScope(context, order)) throw Object.assign(new Error(ADMIN_SCOPE_FORBIDDEN), { statusCode: 403 }); /* detail scope 检查 */ return toReservation(order); }
export async function reserveDelivery(id: string, input: { provider: DeliveryProvider; delivery_mode: DeliveryMode; remark?: string }, actor: Actor, context: AdminAccessContext) {
