...
export async function getFinanceOrders(query: Query) {
  ...
  return { total, page, page_size: pageSize, items: rows.map((order) => ({ order_id: order.id, order_no: order.order_no, user_id: order.user_id, group_buy_id: order.group_buy_id, community_id: order.community_id, pickup_store_id: order.pickup_store_id, order_status: order.order_status, payment_status: order.pay_status, refund_status: order.refund_status, amount: order.total_amount_cents, paid_amount: order.pay_amount_cents, refunded_amount: order.refund_amount_cents, net_amount: order.pay_amount_cents - order.refund_amount_cents, after_sale_case_count: order.after_sale_cases.length, commission_reward_amount: sum(order.commissions.map((item) => item.final_amount_cents)), created_at: order.created_at, paid_at: order.paid_at, completed_at: order.completed_at })) };
}
