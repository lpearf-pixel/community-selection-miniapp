...
function listItem(order: any) {
  ...
  return {
    ...
    delivery_fee_cents: order.pickup_type === 'delivery' ? 0 : 0,
    delivery_time_window_text: order.pickup_type === 'delivery' ? '以门店确认时段为准' : null,
    service_radius_text: order.pickup_type === 'delivery' ? '门店周边 3-5km，具体以门店确认为准' : null,
