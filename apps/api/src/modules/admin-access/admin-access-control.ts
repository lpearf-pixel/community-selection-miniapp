...
function headerIds(
  ...
  const values = [
    headerValue(request, singleName),
    headerValue(request, multiName),
  ]
...
function headerMockAdminDataScope(request: FastifyRequest): AdminDataScope {
  return {
    pickup_store_ids: headerIds(
      request,
      "x-admin-pickup-store-id",
      "x-admin-pickup-store-ids",
    ),
    community_ids: headerIds(
      request,
      "x-admin-community-id",
      "x-admin-community-ids",
    ),
    can_access_all_pickup_stores: false,
    can_access_all_communities: false,
  };
...
export function hasAdminPermission(
  ...
  return (
    ...
    context.permissions.includes("admin.full_access") ||
...
export function requireAdminPermission(
  ...
  return async function adminPermissionGuard(
    request: FastifyRequest,
    reply: FastifyReply,
  ) {
    ...
    if (!adminUser || adminUser.status !== "active") {
      reply
        .code(401)
        .send(fail("ADMIN_UNAUTHORIZED: Active AdminUser required"));
      return;
    }
...
export function canAccessPickupStore(
  context: AdminAccessContext,
  ...
  return (
    !!pickupStoreId &&
    context.data_scope.pickup_store_ids.includes(pickupStoreId)
  );
}
...
export function canAccessCommunity(
  ...
  return (
    !!communityId && context.data_scope.community_ids.includes(communityId)
  );
}

export function canAccessOrderDataScope(
  context: AdminAccessContext,
...
export function getScopedPickupStoreWhere(
  context: AdminAccessContext,
...
export function getScopedCommunityWhere(
  context: AdminAccessContext,
): Record<string, unknown> | null {
...
export function getScopedOrderWhere(
  context: AdminAccessContext,
): Record<string, unknown> | null {
  const or: Record<string, unknown>[] = [];
  if (hasAllPickupStoreScope(context) || hasAllCommunityScope(context))
    return {};
  if (context.data_scope.pickup_store_ids.length > 0)
    or.push({ pickup_store_id: { in: context.data_scope.pickup_store_ids } });
  if (context.data_scope.community_ids.length > 0)
    or.push({ community_id: { in: context.data_scope.community_ids } });
  return or.length > 0 ? { OR: or } : null;
}

export function requireAdminDataScopeForPickupStore(
  context: AdminAccessContext,
  pickupStoreId?: string | null,
): void {
  if (!canAccessPickupStore(context, pickupStoreId))
    throw Object.assign(new Error(ADMIN_SCOPE_FORBIDDEN), { statusCode: 403 });
}

export function requireAdminDataScopeForCommunity(
  context: AdminAccessContext,
  communityId?: string | null,
): void {
  if (!canAccessCommunity(context, communityId))
    throw Object.assign(new Error(ADMIN_SCOPE_FORBIDDEN), { statusCode: 403 });
}
