...
describe('L4 group-buy and order routes', () => {
  ...
  it('registers L12 fulfillment and operations routes', () => {
    ...
    const adminOrderRoutes = readFileSync(new URL('../src/routes/admin/orders.ts', import.meta.url), 'utf8');
    const orderService = readFileSync(new URL('../src/modules/order/order-service.ts', import.meta.url), 'utf8');
    ...
    expect(adminOrderRoutes.includes('/api/admin/orders/:id/pickup-verify')).toBe(true);
    expect(fulfillmentRoutes.includes('/api/admin/orders/:id/pickup-verify')).toBe(false);
    expect(fulfillmentRoutes.includes('pickupVerify')).toBe(false);
    expect(orderService.includes('pickupVerify')).toBe(false);
    expect(orderService.includes("order_status: 'picked'")).toBe(false);
