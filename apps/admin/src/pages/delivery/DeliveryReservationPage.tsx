...
export function DeliveryReservationPage() {
  ...
  async function load() { setLoading(true); try { const [providerRows, orderRows] = await Promise.all([getDeliveryProviders(), getDeliveryOrders({ keyword, page_size: 50 })]); setProviders(providerRows.items); setItems(orderRows.items); } catch (error) { message.error(error instanceof Error ? error.message : "配送预留加载失败"); } finally { setLoading(false); } }
  useEffect(() => { void load(); }, []);
  ...
  async function reserve(row: DeliveryReservation) {
    ...
    message.success("配送预留已保存"); await load();
  }
  async function updateStatus(row: DeliveryReservation) {
    ...
    message.success("配送状态已更新"); await load();
  }

  return <Space direction="vertical" style={{ width: "100%" }}>
    <Card title="配送预留">
      <Alert type="info" showIcon message="配送服务预留" description="门店配送/人工配送为 mock；达达配送接口已预留，当前不会创建真实配送单。页面仅展示 receiver_phone_masked 与 receiver_address_masked。" />
      <Space wrap style={{ marginTop: 16 }}>{providers.map((item) => <Card key={item.provider} size="small" title={item.name}><Tag color={item.enabled ? "green" : "default"}>{item.enabled ? "enabled" : "disabled"}</Tag><Tag>{item.mode}</Tag><Typography.Text>{item.description ?? "mock enabled"}</Typography.Text></Card>)}</Space>
      <Form layout="inline" style={{ marginTop: 16 }} onFinish={load}><Form.Item label="关键词"><Input value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="订单号/收货人" /></Form.Item><Button htmlType="submit" loading={loading}>查询</Button></Form>
    </Card>
    <Card title="配送订单列表">
