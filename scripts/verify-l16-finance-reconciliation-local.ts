...
async function main() {
  ...
  for (const forbidden of ['parent_leader_id', 'upline_id', 'downline', 'team_id', 'level commission']) assert(!rewardText.includes(forbidden), `reward response should not include ${forbidden}`);
  ...
  for (const forbidden of ['parent_leader_id', 'upline_id', 'downline', 'team_id', 'level commission', 'AUTO_PAYOUT_ENABLED = true', 'AUTO_TAX_FILING_ENABLED = true']) assert(!scan.includes(forbidden), `compliance scan should not include ${forbidden}`);
