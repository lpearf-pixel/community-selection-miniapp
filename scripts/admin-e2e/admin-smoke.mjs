...
const navigation = [
  ...
  '自提工作台', '配送预留', '管理配送规则', '老会员导入', '会员权益履约',
];
...
try {
  ...
  assert.ok(
    [
      ...
      'ADMIN_REFUND_VERSION_CONFLICT',
    ].includes(refundConflictEnvelope.code),
    `unexpected refund conflict code: ${refundConflictEnvelope.code}`,
  );
