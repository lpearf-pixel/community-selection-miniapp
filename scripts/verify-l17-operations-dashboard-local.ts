...
async function main() {
  ...
  const reviewed = await adminJson(await app.inject({ method: 'POST', url: `/api/admin/after-sales/${afterSale.id}/review`, headers: adminHeaders, payload: { status: 'approved', approved_refund_cents: 800, approved_product_refund_cents: 800, resolution_type: 'partial_refund', responsibility: 'supplier', admin_note: 'L17 审核' } }));
  const resolved = await adminJson(await app.inject({ method: 'POST', url: `/api/admin/after-sales/${afterSale.id}/refund-execute`, headers: adminHeaders, payload: { expected_version: reviewed.order.version, idempotency_key: `${prefix}-refund-execute`, admin_remark: 'L17 退款' } }));
