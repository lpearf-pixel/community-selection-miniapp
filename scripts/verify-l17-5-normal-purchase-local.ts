...
async function main() {
  ...
  const reviewed = await json(await app.inject({ method: 'POST', url: `/api/admin/after-sales/${afterSale.id}/review`, headers: adminHeaders, payload: { status: 'approved', resolution_type: 'partial_refund', approved_refund_cents: 500, approved_product_refund_cents: 500, responsibility: 'platform', admin_note: 'L17.5 normal purchase after-sale approved' } }));
  await json(await app.inject({ method: 'POST', url: `/api/admin/after-sales/${afterSale.id}/refund-execute`, headers: adminHeaders, payload: { expected_version: reviewed.order.version, idempotency_key: `${prefix}-refund-execute`, admin_remark: 'L17.5 normal purchase after-sale resolved' } }));
