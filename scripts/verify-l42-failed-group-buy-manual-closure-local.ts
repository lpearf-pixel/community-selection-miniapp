...
async function main() {
  ...
  const failed = await markGroupBuyFailed({ group_buy_id: groupBuy.id, reason: 'L42 人工确认未达成团条件', admin_meta: { admin_user_id: 'l42-admin' } });
  assert(failed.status === 'failed' && failed.applied, 'pending group buy must become failed');
  const failedAgain = await markGroupBuyFailed({ group_buy_id: groupBuy.id, reason: 'L42 repeat', admin_meta: { admin_user_id: 'l42-admin' } });
  ...
  try { await markGroupBuyFailed({ group_buy_id: successGroup.id, reason: 'should reject' }); } catch { successRejected = true; }
  assert(successRejected, 'success group buy cannot be failed');

  const closeUnpaid = await closeFailedGroupBuyUnpaidOrders({ group_buy_id: groupBuy.id, admin_meta: { admin_user_id: 'l42-admin' } });
  ...
  try { await confirmFailedGroupBuyRefundHandled({ group_buy_id: groupBuy.id, order_id: paidOrder.id, refund_id: pendingRefund.id }); } catch { pendingRejected = true; }
  assert(pendingRejected, 'pending refund cannot be confirmed');
  const blocked = await closeFailedGroupBuy({ group_buy_id: groupBuy.id });
  ...
  await confirmFailedGroupBuyRefundHandled({ group_buy_id: groupBuy.id, order_id: paidOrder.id, refund_id: successRefund.id, admin_meta: { admin_user_id: 'l42-admin' } });
  await confirmFailedGroupBuyRefundHandled({ group_buy_id: groupBuy.id, order_id: paidOrder.id, refund_id: successRefund.id, admin_meta: { admin_user_id: 'l42-admin' } });
  ...
  const closed = await closeFailedGroupBuy({ group_buy_id: groupBuy.id, admin_meta: { admin_user_id: 'l42-admin' } });
  assert(closed.applied && closed.status === 'closed', 'final close must set closed');
  const closedAgain = await closeFailedGroupBuy({ group_buy_id: groupBuy.id, admin_meta: { admin_user_id: 'l42-admin' } });
  assert(closedAgain.idempotent && closedAgain.status === 'closed', 'repeat final close idempotent');

  const inactiveAdminId = `${prefix}-inactive-admin`;
  const scopedAdminId = `${prefix}-scoped-admin`;
  await prisma.adminUser.create({ data: { id: inactiveAdminId, username: `${prefix}-inactive`, password_hash: 'placeholder', role: 'super_admin', status: 'inactive' } });
