...
7. 旧 release 未校验团购有效状态；L43 release 仅释放 pending 且 T+3 到期的有效记录。
...
`Commission` 保存当前聚合状态、计算基数、预估、扣减、最终金额、T+3 可用时间和人工复核字段。`RewardLedger` 是不可变事件账本，使用幂等键记录 available 入账、退款扣减、转换消费额度扣减和人工复核事件。
