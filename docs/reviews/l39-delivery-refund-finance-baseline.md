...
L39 Docker API E2E 需要覆盖一个真实非零配送费拆分退款场景：先配置 `base_fee_cents = 500`，再创建门店配送订单，验证 `pay_amount_cents = product_amount_cents + 500`。完成 mock 支付后，售后申请与审核分别使用 `requested_product_refund_cents = 100`、`requested_delivery_refund_cents = 200`、总退款 `300`，并验证用户订单详情、财务 overview 与退款 CSV 均包含对应拆分金额。

同一 E2E 还必须验证到店自提订单不能申请配送费退款，即 `pickup_type=store` 且 `requested_delivery_refund_cents > 0` 时接口返回失败。
